...
export function FormFieldWrapper({ field, onSelect, removeField }: FormFieldWrapperProps) {
  const {
    attributes,
    listeners,
    setNodeRef,
    ...
    <div ref={setNodeRef} style={style} className={isDragging ? 'relative' : ''}>
      <Card className={cn(
        ...
        <CardContent className="p-4 pl-12">
          ...
          {field.placeholder &&
            <p className="text-sm text-muted-foreground mt-2 ml-9">{field.placeholder}</p>
          }
        </CardContent>
      </Card>
    </div>
